...
class ObjectDetection:
	...
	def __init__(self):
		...
		self.model.conf = 0.6 # set inference threshold at 0.3
		self.model.iou = 0.5 # set inference IOU threshold at 0.3
	...
	def __call__(self):
		...
		"""
		model_stream = threading.Thread(target = modeling, args=frame)
		x_shape = 400
		y_shape = 350
		WIDTH =400
		frame_skip = 300
		...
		for frame in player:
			modeling(frame).start()
			frame = cv2.cvtColor(np.array(frame.to_image()), cv2.COLOR_RGB2BGR)
			frame = imutils.resize(frame,width=WIDTH)
			frame = self.plot_boxes(self.results, frame)
